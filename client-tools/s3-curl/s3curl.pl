...
#NOTE: This is a modified s3curl.pl specifically to support Eucalyptus/Walrus.
# Support for the --endpoint option was added as well as removing the annoying
# 'key on command line' warning and making it shorter and faster to facilitate QA.
# Modified by: Zach Hill 3-23-2012
...
GetOptions(
    ...
    'help' => \$help,
    'debug' => \$debug,
    'endpoint=s' => \$usr_endpoint
);

my $usage = <<USAGE;
Usage $0 --id friendly-name (or AWSAccessKeyId) [options] -- [curl-options] [URL]
 options:
  --endpoint endpoint_url     ip or url of endpoint to use (not required if using a default AWS S3 endpoing)
...
if ($cmdLineSecretKey) {
    ...
    #sleep 5;
    print STDERR "WARNING: You shouldn't put secret key on cmd line!\n";
...
# Add the endpoint if the user specified one
if (defined $usr_endpoint) {
  debug("User specified $usr_endpoint as an endpoint. Adding it to the list of valid endpoints.");
  push(@endpoints, $usr_endpoint);
}
